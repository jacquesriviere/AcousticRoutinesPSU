function [max_interpolation,timedelay,min_interpolation] = delay(X,ts)
...
% It also finds the minimum of intercorrelation. When two waveforms are
% very much out of phase, the minimum of intercorrelation will be larger
...
% interpolate minimum as well
% When changes are large from one WF to another (e.g. when they become out
% of phase), the minimum will be larger that the maximum in absolute value
[mini,ind2] = min(X);

% parabolic interpolation around the max
x1 = ind2 - 1; x2 = ind2; x3 = ind2 + 1;
...
ind_min_corr = -b/(2*a);
min_interpolation = a*ind_min_corr^2 + b*ind_min_corr + c;

timedelay_min = ts*(ind_min_corr-NN);

interpolationXmin = x1:1/100:x3;
interpolationYmin = a*interpolationXmin.^2 + b*interpolationXmin + c;
interpolationtmin = ts*(interpolationXmin-NN);

% if abs(min_interpolation) > max_interpolation % plot only when WFs are very different
...
%% uncomment to see the fitting result
    ...
    plot(tps(ind2),mini,'*m'); % max of intercorrelation
    plot(timedelay_min,min_interpolation,'*g'); % refined max of intercorrelation
    plot(interpolationtmin,interpolationYmin,'g');
